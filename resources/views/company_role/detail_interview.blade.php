<x-app-layout>
        ...
        <div class="flex">
            <div class="py-2 bg-white shadow-lg rounded-lg mx-7 p-5 w-2/6">
                <div class="bg-slate-200 shadow-lg p-4 mt-8 flex items-center justify-center rounded-xl">
                    <h1 class="font-bold">DATA PERUSAHAAN</h1><input type="hidden" id="code"
            ...
            <div class="w-full md:w-1/2 relative bg-white rounded-lg shadow mx-5">
                ...
                <form method="POST" id="formSourceModal">
                    ...
                    <div class="p-4 space-y-6">
                        ...
                        <div>
                            <label for="status"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status Lowongan
                                <span class="text-red-500">*</span></label>
                            <select class="js-example-placeholder-single w-full" id="status" name="status"
    ...
    @push('scripts')
        <script>
            ...
            let urlFollowup = `/api/detail/${id}/${status}`;

            // Example function to make the AJAX call
            function fetchDetail() {
                fetch(urlFollowup)
                    ...
                    .then(data => {
                        ...
                        console.log(data);
                        // Initialize or update the data table here
                    })
                    .catch(error => {
                            ...
                            },
                            {
                                ...
                                render: (data) => {
                                    ...
                                    <a href="#" data-id="${data.id}"
                                                        data-modal-target="sourceModal" data-status="${data.status}" data-nim="${data.nim}"
                                                        onclick="return editSourceModal(${data.id}, '${data.status}', '${data.nim}')" class="bg-white hover:bg-black hover:text-white px-4 py-2 border border-2 border-black rounded-l-full text-xs">
                                        Terima
                                    </a>`;
                                    let tolakUrl = `
                                    <a href="#" data-id="${data.id}"
                                                        data-modal-target="sourceModal" data-status="${data.status}" data-nim="${data.nim}"
                                                        onclick="return editSourceModalTolak(${data.id}, '${data.status}', '${data.nim}')" class="bg-white hover:bg-black hover:text-white px-4 py-2 border border-2 border-black rounded-r-full text-xs">
                                        Tolak
                                    </a>`;
                                    return `
        ...
        <script>
            const editSourceModal = (id, status, nim) => {
                console.log(id,status,nim);
                const formModal = document.getElementById('formSourceModal');
                const sourceModal = document.getElementById('sourceModal');
                let url = "{{ route('interview.terima', ':id') }}".replace(':id', id);
                document.getElementById('formSourceModal').setAttribute('action', url);
                document.getElementById('nim').value=nim;

                document.getElementById('title_source').innerText = 'Konfirmasi';
                document.getElementById('formSourceButton').innerText = 'Terima';
            ...
            const editSourceModalTolak = (id, status, nim) => {
                const formModal = document.getElementById('formSourceModal');
                const sourceModal = document.getElementById('sourceModal');
                let url = "{{ route('interview.tolak', ':id') }}".replace(':id', id);
                document.getElementById('formSourceModal').setAttribute('action', url);
                document.getElementById('nim').value=nim;
